...
def conv2d(x, filters, kernel_size, stride=1, padding="same",
    ...
    return tf.keras.layers.conv2d(x, filters, kernel_sizes, strides=strides,
...
def max_pool2d(x, pool_size, stride=None, scope="max_pool2d"):
    ...
    return tf.keras.layers.max_pooling2d(x, pool_sizes, strides, name=scope, padding="same")
...
def dropout(x, rate=0.5, is_training=True):
    return tf.keras.layers.dropout(x, rate=rate, training=is_training)

# l2norm (not bacth norm, spatial normalization)
def l2norm(x, scale, trainable=True, scope="L2Normalization"):
    n_channels = x.get_shape().as_list()[-1]
    l2_norm = tf.math.l2_normalize(x, [3], epsilon=1e-12)
