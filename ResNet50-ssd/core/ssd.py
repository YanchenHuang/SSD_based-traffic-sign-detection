...
class SSD(tf.keras.Model):#基于ResNet50的SSD网络
    def __init__(self):
    ...
    def _predict_layer(self, k):#使用一个2D的
    ...
    def call(self, inputs, training=None, mask=None):
        branch_1, x = self.backbone(inputs, training=training)#经过ResNet50产生一个Tensor：[   1   19   19 1024]
        #print("ResNet50:",tf.shape(x),"\n")
...
def ssd_prediction(feature_maps, num_classes):
    ...
    for feature in feature_maps:#一共进行6次循环,对于每个predict_n 进行一次reshape
        print(tf.shape(feature))
        predicted_features_list.append(tf.reshape(tensor=feature, shape=(batch_size, -1, num_classes + 4)))
    predicted_features = tf.concat(values=predicted_features_list, axis=1)#最终将reshape后的每一层的feature map合并在一个tensor中
