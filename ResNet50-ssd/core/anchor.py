...
class DefaultBoxes(object):
    ...
    def __generate_default_boxes_for_one_feature_map(self, feature_map_index):
        ...
        center_x, center_y = x_y_meshgrid(x_row=feature_map_size[1], y_col=feature_map_size[0])#每个featureMap的像素点都作为一个center，产生mesh
        center_x = (center_x + self.offset) / feature_map_size[1]#把feature map上的点映射回原图,这里加上0.5也是为了四舍五入
        center_y = (center_y + self.offset) / feature_map_size[0]
        w = []
        h = []
        for i in range(len(ar)):#在这个循环里，通过不同的ratio来产生不用大小的box，并将box的长款放在w，h中
            # for the aspect ratio of 1
            if ar[i] == 1.0:
                s_k_ar1 = math.sqrt(s_k * s_k1)
                w.append(s_k_ar1)
                h.append(s_k_ar1)
            w.append(s_k * math.sqrt(ar[i]))#见ssd default Box的长款计算公式
        ...
        return cx, cy, w, h#cx，cy分别为这个一个feature Map上的各个像素点，以其为中心。w，h是可能出现的Default Box的长款

    @staticmethod
    def __get_default_boxes_for_single_feature(xy, wh):#将原本的cx，cy，w，h的进行再处理，并最终变成[[x，y，w，h],.....]的形式
        xywh_list = []
    ...
    def generate_default_boxes(self):#在所有的产生的6个featureMap中，产生很多的defaultBox
        feature_map_boxes = []
